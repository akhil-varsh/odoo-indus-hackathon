...
export default function ReceiptsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [list, productData, locationData] = await Promise.all([
        apiRequest<ReceiptResponse>(`/receipts?${query.toString()}`, { token }),
      ...
      if (locationData.length && !form.toLocationId) {
        setForm((old) => ({ ...old, toLocationId: locationData[0].id }));
      }
    } catch (loadError) {
  ...
  const createReceipt = async (event: FormEvent) => {
    ...
    try {
      ...
      setForm((old) => ({ ...old, reference: "", supplier: "", quantity: 1 }));
      await load();
    } catch (createError) {
      setError(createError instanceof Error ? createError.message : "Failed to create receipt");
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <form className="mt-3 grid gap-2 md:grid-cols-5" onSubmit={createReceipt}>
          ...
          <input className="rounded border px-3 py-2" placeholder="Supplier" value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} />
          ...
          <select className="rounded border px-3 py-2" value={form.toLocationId} onChange={(e) => setForm({ ...form, toLocationId: e.target.value })}>
            {locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
          </select>
          <div className="flex gap-2">
            <input className="w-24 rounded border px-3 py-2" type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: Number(e.target.value) })} />
            <button className="rounded bg-emerald-600 px-3 py-2 text-white" type="submit">Create</button>
          </div>
        </form>
      </section>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-slate-100">
                  <td className="py-2 font-semibold text-blue-700"><Link href={`/receipts/${row.id}`}>{row.reference || row.id}</Link></td>
